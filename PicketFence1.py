...
from pylinac.core.geometry import Line, Rectangle, Point
...
##########################################################
    def error_leafs(self) -> int:
        """Return the 3 leaf that had the maximum error."""

        e_array = []

        for picket in self.pickets:
            e_array.append(picket.error_array)

        e_array = np.stack(e_array, axis=0)

        leaf_seq = []

        for i in range(3):
            ind = np.unravel_index(np.argmax(e_array, axis=None), e_array.shape)
            leaf_seq.append(ind[1])
            e_array[:,ind[1]] = 0
